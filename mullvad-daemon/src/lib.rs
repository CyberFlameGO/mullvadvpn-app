...
pub enum DaemonCommand {
    ...
    SetPostQuantumSafety(ResponseTx<(), settings::Error>, bool),
    ...
    L: EventListener + Clone + Send + 'static,
{
    ...
    async fn handle_command(&mut self, command: DaemonCommand) {
        ...
        match command {
            ...
            SetPostQuantumSafety(tx, enable_pq) => {
                self.on_set_post_quantum_safety(tx, enable_pq).await
            }
    ...
    async fn on_set_post_quantum_safety(
        &mut self,
        tx: ResponseTx<(), settings::Error>,
        use_pq_safe_psk: bool,
    ) {
        let save_result = self.settings.set_post_quantum_safety(use_pq_safe_psk).await;
        match save_result {
            Ok(settings_changed) => {
                Self::oneshot_send(tx, Ok(()), "set_post_quantum_safety response");
                if settings_changed {
                    self.parameters_generator
                        .set_tunnel_options(&self.settings.tunnel_options);
                    self.event_listener
            ...
            Err(e) => {
                log::error!("{}", e.display_chain_with_msg("Unable to save settings"));
                Self::oneshot_send(tx, Err(e), "set_post_quantum_safety response");
            }
        }
    }
